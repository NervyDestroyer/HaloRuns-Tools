...
{
vars.indexoffset = 0;
return (current.cinematic == 0 && old.cinematic == 1 && current.tickcounter1 < 860 && current.levelname == "a50"); //starts timer on specific tick that chief gets movement
}
...
//		return (-25 < current.zpos && -14 > current.zpos && -65 < current.ypos && -55 > current.ypos && -14 < current.xpos && -5 > current.xpos);
		...
		return (current.bspstate == 3 && current.fadelength == 15 && current.cutsceneskip == 1 && old.cutsceneskip == 0);
		break;
	}
}
...
reset
{
return (current.tickcounter1 < 845 && current.levelname == "a50");
}