...
{
vars.indexoffset = 0;
return (current.cinematic == 0 && old.cinematic == 1 && current.tickcounter1 < 1080 && current.levelname == 1546662754); //starts timer on specific tick that chief hits the ground
}
...
int checkindex = timer.CurrentSplitIndex + vars.indexoffset;
	switch (checkindex)
	{
		...
		return (current.ypos < -23.3 && current.xpos < 3.7);
		break;
